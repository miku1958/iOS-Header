//
//     Generated by class-dump 3.5-MH (64 bit).
//
//  Copyright (C) 1997-2019 Steve Nygard.
//

#import <objc/NSObject.h>

@class AMSBuyParams, AMSPurchase, NSString;

__attribute__((visibility("hidden")))
@interface VUIPurchaseRequest : NSObject
{
    AMSPurchase *_purchase;
    NSString *_buyParamsString;
    long long _type;
    NSString *_enhancedBuyParamsString;
    AMSBuyParams *_amsBuyParams;
    long long _purchaseType;
    CDUnknownBlockType _completionHandler;
}

@property (strong, nonatomic) AMSBuyParams *amsBuyParams; // @synthesize amsBuyParams=_amsBuyParams;
@property (strong, nonatomic) NSString *buyParamsString; // @synthesize buyParamsString=_buyParamsString;
@property (copy, nonatomic) CDUnknownBlockType completionHandler; // @synthesize completionHandler=_completionHandler;
@property (strong, nonatomic) NSString *enhancedBuyParamsString; // @synthesize enhancedBuyParamsString=_enhancedBuyParamsString;
@property (strong, nonatomic) AMSPurchase *purchase; // @synthesize purchase=_purchase;
@property (nonatomic) long long purchaseType; // @synthesize purchaseType=_purchaseType;
@property (nonatomic) long long type; // @synthesize type=_type;

+ (long long)getPurchaseTypeFromActionRef:(id)arg1;
+ (BOOL)shouldPlayWhenDoneForActionRef:(id)arg1;
- (void).cxx_destruct;
- (void)_recordErrorLog:(id)arg1;
- (void)enqueueWithCompletion:(CDUnknownBlockType)arg1;
- (void)handleRequestCompletionWithResult:(id)arg1 andError:(id)arg2;
- (id)initWithBuyParams:(id)arg1 ofPurchaseType:(long long)arg2;

@end

