//
//     Generated by class-dump 3.5-MH (64 bit).
//
//  Copyright (C) 1997-2019 Steve Nygard.
//

#import <objc/NSObject.h>

#import <Silex/SXComponentInsertionCondition-Protocol.h>

@class NSMutableIndexSet, NSString;

@interface SXTraitDistanceComponentInsertionCondition : NSObject <SXComponentInsertionCondition>
{
    unsigned long long _componentTrait;
    unsigned long long _otherComponentTrait;
    NSMutableIndexSet *_traitAreas;
    NSMutableIndexSet *_otherTraitAreas;
    struct _SXConvertibleValue _distance;
}

@property (readonly, nonatomic) unsigned long long componentTrait; // @synthesize componentTrait=_componentTrait;
@property (readonly, copy) NSString *debugDescription;
@property (readonly, copy) NSString *description;
@property (readonly, nonatomic) struct _SXConvertibleValue distance; // @synthesize distance=_distance;
@property (readonly) unsigned long long hash;
@property (readonly, nonatomic) unsigned long long otherComponentTrait; // @synthesize otherComponentTrait=_otherComponentTrait;
@property (strong, nonatomic) NSMutableIndexSet *otherTraitAreas; // @synthesize otherTraitAreas=_otherTraitAreas;
@property (readonly) Class superclass;
@property (strong, nonatomic) NSMutableIndexSet *traitAreas; // @synthesize traitAreas=_traitAreas;

- (void).cxx_destruct;
- (id)initWithTrait:(unsigned long long)arg1 otherTrait:(unsigned long long)arg2 distance:(struct _SXConvertibleValue)arg3;
- (void)insertedComponent:(id)arg1 approximateLocation:(struct CGPoint)arg2;
- (void)inspectTraitAreasForComponents:(id)arg1 layoutProvider:(id)arg2;
- (void)prepareWithComponents:(id)arg1 layoutProvider:(id)arg2;
- (BOOL)validateMarker:(id)arg1 componentTraits:(unsigned long long)arg2 layoutProvider:(id)arg3;

@end

