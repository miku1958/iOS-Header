//
//     Generated by class-dump 3.5-MH (64 bit).
//
//  Copyright (C) 1997-2019 Steve Nygard.
//

#import <objc/NSObject.h>

#import <TrackingAvoidance/NSSecureCoding-Protocol.h>

@class TADeviceRecord, TAVisitState;

@interface TAPersistenceStore : NSObject <NSSecureCoding>
{
    TADeviceRecord *_deviceRecord;
    TAVisitState *_visitState;
}

@property (strong, nonatomic) TADeviceRecord *deviceRecord; // @synthesize deviceRecord=_deviceRecord;
@property (strong, nonatomic) TAVisitState *visitState; // @synthesize visitState=_visitState;

+ (BOOL)supportsSecureCoding;
- (void).cxx_destruct;
- (void)encodeWithCoder:(id)arg1;
- (id)initWithCoder:(id)arg1;
- (BOOL)isEqual:(id)arg1;

@end

