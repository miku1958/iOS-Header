//
//     Generated by class-dump 3.5-MH (64 bit).
//
//  Copyright (C) 1997-2019 Steve Nygard.
//

#import <objc/NSObject.h>

@class NSArray, NSMutableDictionary;

@interface REMAuxiliaryChangeInfoFetchResult : NSObject
{
    NSArray *_auxiliaryChangeInfos;
    Class _typedKlass;
    NSMutableDictionary *_changeObjectForAuxiliaryChangeInfoMap;
}

@property (strong, nonatomic) NSArray *auxiliaryChangeInfos; // @synthesize auxiliaryChangeInfos=_auxiliaryChangeInfos;
@property (strong, nonatomic) NSMutableDictionary *changeObjectForAuxiliaryChangeInfoMap; // @synthesize changeObjectForAuxiliaryChangeInfoMap=_changeObjectForAuxiliaryChangeInfoMap;
@property (nonatomic) Class typedKlass; // @synthesize typedKlass=_typedKlass;

+ (id)auxiliaryChangeInfoFetchResultOfType:(Class)arg1;
- (void).cxx_destruct;
- (id)auxiliaryChangeInfoFromData:(id)arg1 withObjectID:(id)arg2 fromChangeObject:(id)arg3 error:(id *)arg4;
- (id)changeObjectForAuxiliaryChangeInfo:(id)arg1;
- (id)initWithAuxiliaryChangeInfoType:(Class)arg1;

@end

