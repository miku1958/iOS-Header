//
//     Generated by class-dump 3.5-MH (64 bit).
//
//  Copyright (C) 1997-2019 Steve Nygard.
//

#import <objc/NSObject.h>

@class NSArray, NSMutableSet, NSSet;

@interface REMChangedKeysObserver : NSObject
{
    NSObject *_target;
    NSArray *_keysToObserve;
    NSMutableSet *_mutableChangedKeys;
}

@property (readonly, nonatomic) NSSet *changedKeys;
@property (strong, nonatomic) NSArray *keysToObserve; // @synthesize keysToObserve=_keysToObserve;
@property (strong, nonatomic) NSMutableSet *mutableChangedKeys; // @synthesize mutableChangedKeys=_mutableChangedKeys;
@property (readonly, nonatomic) NSObject *target; // @synthesize target=_target;

- (void).cxx_destruct;
- (void)dealloc;
- (id)initWithTarget:(id)arg1 keysToObserve:(id)arg2 includeInitial:(BOOL)arg3;
- (void)keyDidChange:(id)arg1;
- (void)observeValueForKeyPath:(id)arg1 ofObject:(id)arg2 change:(id)arg3 context:(void *)arg4;

@end

