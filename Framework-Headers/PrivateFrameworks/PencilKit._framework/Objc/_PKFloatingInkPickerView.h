//
//     Generated by class-dump 3.5-MH (64 bit).
//
//  Copyright (C) 1997-2019 Steve Nygard.
//

#import <UIKit/UIView.h>

@class PKInlineInkPicker, UIButton, UIVisualEffectView;

@interface _PKFloatingInkPickerView : UIView
{
    unsigned long long _position;
    UIVisualEffectView *_dockView;
    PKInlineInkPicker *_inkPicker;
    UIButton *_undoButton;
    UIButton *_redoButton;
    UIButton *_shapeButton;
    struct CGPoint _dragStartOrigin;
}

@property (strong, nonatomic) UIVisualEffectView *dockView; // @synthesize dockView=_dockView;
@property (nonatomic) struct CGPoint dragStartOrigin; // @synthesize dragStartOrigin=_dragStartOrigin;
@property (strong, nonatomic) PKInlineInkPicker *inkPicker; // @synthesize inkPicker=_inkPicker;
@property (nonatomic) unsigned long long position; // @synthesize position=_position;
@property (strong, nonatomic) UIButton *redoButton; // @synthesize redoButton=_redoButton;
@property (strong, nonatomic) UIButton *shapeButton; // @synthesize shapeButton=_shapeButton;
@property (strong, nonatomic) UIButton *undoButton; // @synthesize undoButton=_undoButton;

- (void).cxx_destruct;
- (void)didPanInkPicker:(id)arg1;
- (id)hitTest:(struct CGPoint)arg1 withEvent:(id)arg2;
- (id)initWithFrame:(struct CGRect)arg1;
- (void)layoutSubviews;
- (void)setPositionForProjectedOrigin:(struct CGPoint)arg1;

@end

