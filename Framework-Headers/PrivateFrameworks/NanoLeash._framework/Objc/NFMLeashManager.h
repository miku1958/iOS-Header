//
//     Generated by class-dump 3.5-MH (64 bit).
//
//  Copyright (C) 1997-2019 Steve Nygard.
//

#import <objc/NSObject.h>

#import <NanoLeash/CBCentralManagerDelegate-Protocol.h>
#import <NanoLeash/CBPeripheralDelegate-Protocol.h>
#import <NanoLeash/CBScalablePipeManagerDelegate-Protocol.h>

@class CBCentralManager, CBPeripheral, CBScalablePipe, CBScalablePipeManager, NSString;
@protocol NFMLLeashBreakDelegate, OS_dispatch_queue, OS_dispatch_source;

@interface NFMLeashManager : NSObject <CBCentralManagerDelegate, CBPeripheralDelegate, CBScalablePipeManagerDelegate>
{
    CDUnknownBlockType _proximityChangeHandler;
    NSObject<OS_dispatch_queue> *_centralQueue;
    BOOL _testMode;
    BOOL _connected;
    BOOL _needsToScanForPeripheral;
    long long _leashMode;
    NSString *_serviceUUID;
    NSString *_leashIdentifier;
    CBScalablePipeManager *_pipeManager;
    CBScalablePipe *_pipe;
    CBCentralManager *_centralManager;
    CBPeripheral *_peripheral;
    CBPeripheral *_discoveredPeripheral;
    long long _centralPhase;
    id<NFMLLeashBreakDelegate> _leashDelegate;
    NSObject<OS_dispatch_source> *_leashDispatchSourceTimer;
    NSObject<OS_dispatch_queue> *_externalQueue;
}

@property (strong, nonatomic) CBCentralManager *centralManager; // @synthesize centralManager=_centralManager;
@property (nonatomic) long long centralPhase; // @synthesize centralPhase=_centralPhase;
@property (nonatomic) BOOL connected; // @synthesize connected=_connected;
@property (readonly, copy) NSString *debugDescription;
@property (readonly, copy) NSString *description;
@property (strong, nonatomic) CBPeripheral *discoveredPeripheral; // @synthesize discoveredPeripheral=_discoveredPeripheral;
@property (strong, nonatomic) NSObject<OS_dispatch_queue> *externalQueue; // @synthesize externalQueue=_externalQueue;
@property (readonly) unsigned long long hash;
@property (weak, nonatomic) id<NFMLLeashBreakDelegate> leashDelegate; // @synthesize leashDelegate=_leashDelegate;
@property (strong, nonatomic) NSObject<OS_dispatch_source> *leashDispatchSourceTimer; // @synthesize leashDispatchSourceTimer=_leashDispatchSourceTimer;
@property (copy, nonatomic) NSString *leashIdentifier; // @synthesize leashIdentifier=_leashIdentifier;
@property (nonatomic) long long leashMode; // @synthesize leashMode=_leashMode;
@property (nonatomic) BOOL needsToScanForPeripheral; // @synthesize needsToScanForPeripheral=_needsToScanForPeripheral;
@property (strong, nonatomic) CBPeripheral *peripheral; // @synthesize peripheral=_peripheral;
@property (strong, nonatomic) CBScalablePipe *pipe; // @synthesize pipe=_pipe;
@property (strong, nonatomic) CBScalablePipeManager *pipeManager; // @synthesize pipeManager=_pipeManager;
@property (copy, nonatomic) NSString *serviceUUID; // @synthesize serviceUUID=_serviceUUID;
@property (readonly) Class superclass;
@property (nonatomic) BOOL testMode; // @synthesize testMode=_testMode;

- (void).cxx_destruct;
- (void)beginMonitoringLeash;
- (void)centralManager:(id)arg1 didConnectPeripheral:(id)arg2;
- (void)centralManager:(id)arg1 didDiscoverPeripheral:(id)arg2 advertisementData:(id)arg3 RSSI:(id)arg4;
- (void)centralManager:(id)arg1 didFailToConnectPeripheral:(id)arg2 error:(id)arg3;
- (void)centralManagerDidUpdateState:(id)arg1;
- (void)connect;
- (void)dealloc;
- (id)init;
- (void)leashTest;
- (void)peripheralDidUpdateRSSI:(id)arg1 error:(id)arg2;
- (void)scalablePipeManager:(id)arg1 didRegisterEndpoint:(id)arg2 error:(id)arg3;
- (void)scalablePipeManager:(id)arg1 didUnregisterEndpoint:(id)arg2;
- (void)scalablePipeManager:(id)arg1 pipeDidConnect:(id)arg2;
- (void)scalablePipeManager:(id)arg1 pipeDidDisconnect:(id)arg2 error:(id)arg3;
- (void)scalablePipeManagerDidUpdateState:(id)arg1;
- (void)setDelegate:(id)arg1 queue:(id)arg2;
- (void)setProximityChangeHandler:(CDUnknownBlockType)arg1;
- (void)stopMonitoringLeash;
- (void)update:(id)arg1;

@end

