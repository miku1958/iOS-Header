//
//     Generated by class-dump 3.5-MH (64 bit).
//
//  Copyright (C) 1997-2019 Steve Nygard.
//

#import <objc/NSObject.h>

#import <CoreSuggestionsInternals/MLFeatureProvider-Protocol.h>

@class MLMultiArray, NSSet;

__attribute__((visibility("hidden")))
@interface SGReminderModelCoreMLOutputWrapper : NSObject <MLFeatureProvider>
{
    MLMultiArray *_polarity;
    MLMultiArray *_trigger;
    MLMultiArray *_title;
    MLMultiArray *_bi_lstm_0_h_out;
    MLMultiArray *_bi_lstm_0_c_out;
    MLMultiArray *_bi_lstm_0_h_out_rev;
    MLMultiArray *_bi_lstm_0_c_out_rev;
    MLMultiArray *_bi_lstm_1_h_out;
    MLMultiArray *_bi_lstm_1_c_out;
    MLMultiArray *_bi_lstm_1_h_out_rev;
    MLMultiArray *_bi_lstm_1_c_out_rev;
}

@property (strong, nonatomic) MLMultiArray *bi_lstm_0_c_out; // @synthesize bi_lstm_0_c_out=_bi_lstm_0_c_out;
@property (strong, nonatomic) MLMultiArray *bi_lstm_0_c_out_rev; // @synthesize bi_lstm_0_c_out_rev=_bi_lstm_0_c_out_rev;
@property (strong, nonatomic) MLMultiArray *bi_lstm_0_h_out; // @synthesize bi_lstm_0_h_out=_bi_lstm_0_h_out;
@property (strong, nonatomic) MLMultiArray *bi_lstm_0_h_out_rev; // @synthesize bi_lstm_0_h_out_rev=_bi_lstm_0_h_out_rev;
@property (strong, nonatomic) MLMultiArray *bi_lstm_1_c_out; // @synthesize bi_lstm_1_c_out=_bi_lstm_1_c_out;
@property (strong, nonatomic) MLMultiArray *bi_lstm_1_c_out_rev; // @synthesize bi_lstm_1_c_out_rev=_bi_lstm_1_c_out_rev;
@property (strong, nonatomic) MLMultiArray *bi_lstm_1_h_out; // @synthesize bi_lstm_1_h_out=_bi_lstm_1_h_out;
@property (strong, nonatomic) MLMultiArray *bi_lstm_1_h_out_rev; // @synthesize bi_lstm_1_h_out_rev=_bi_lstm_1_h_out_rev;
@property (readonly, nonatomic) NSSet *featureNames;
@property (strong, nonatomic) MLMultiArray *polarity; // @synthesize polarity=_polarity;
@property (strong, nonatomic) MLMultiArray *title; // @synthesize title=_title;
@property (strong, nonatomic) MLMultiArray *trigger; // @synthesize trigger=_trigger;

- (void).cxx_destruct;
- (id)featureValueForName:(id)arg1;
- (id)initWithPolarity:(id)arg1 trigger:(id)arg2 title:(id)arg3 bi_lstm_0_h_out:(id)arg4 bi_lstm_0_c_out:(id)arg5 bi_lstm_0_h_out_rev:(id)arg6 bi_lstm_0_c_out_rev:(id)arg7 bi_lstm_1_h_out:(id)arg8 bi_lstm_1_c_out:(id)arg9 bi_lstm_1_h_out_rev:(id)arg10 bi_lstm_1_c_out_rev:(id)arg11;

@end

