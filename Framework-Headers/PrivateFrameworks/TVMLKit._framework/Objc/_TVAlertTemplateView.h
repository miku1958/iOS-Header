//
//     Generated by class-dump 3.5-MH (64 bit).
//
//  Copyright (C) 1997-2019 Steve Nygard.
//

#import <UIKit/UIView.h>

@class NSArray, UICollectionView, UIImage, UIImageView;

__attribute__((visibility("hidden")))
@interface _TVAlertTemplateView : UIView
{
    UIImage *_bgImage;
    UIImageView *_bgImageView;
    NSArray *_viewsAbove;
    UICollectionView *_collectionView;
    NSArray *_viewsBelow;
    struct UIEdgeInsets _padding;
}

@property (strong, nonatomic) UICollectionView *collectionView; // @synthesize collectionView=_collectionView;
@property (nonatomic) struct UIEdgeInsets padding; // @synthesize padding=_padding;
@property (strong, nonatomic) NSArray *viewsAbove; // @synthesize viewsAbove=_viewsAbove;
@property (strong, nonatomic) NSArray *viewsBelow; // @synthesize viewsBelow=_viewsBelow;

- (void).cxx_destruct;
- (void)layoutSubviews;
- (void)setBgImage:(id)arg1;

@end

