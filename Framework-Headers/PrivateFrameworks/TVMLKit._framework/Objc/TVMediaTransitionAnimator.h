//
//     Generated by class-dump 3.5-MH (64 bit).
//
//  Copyright (C) 1997-2019 Steve Nygard.
//

#import <objc/NSObject.h>

#import <TVMLKit/UIViewControllerAnimatedTransitioning-Protocol.h>

@class NSString;
@protocol TVMediaControllerHosting;

@interface TVMediaTransitionAnimator : NSObject <UIViewControllerAnimatedTransitioning>
{
    BOOL _migrates;
    id<TVMediaControllerHosting> _fromHost;
    id<TVMediaControllerHosting> _toHost;
}

@property (readonly, copy) NSString *debugDescription;
@property (readonly, copy) NSString *description;
@property (strong, nonatomic) id<TVMediaControllerHosting> fromHost; // @synthesize fromHost=_fromHost;
@property (readonly) unsigned long long hash;
@property (readonly) Class superclass;
@property (strong, nonatomic) id<TVMediaControllerHosting> toHost; // @synthesize toHost=_toHost;

- (void).cxx_destruct;
- (void)animateTransition:(id)arg1;
- (void)migratesMediaControllerFromHost:(id)arg1 toHost:(id)arg2;
- (double)transitionDuration:(id)arg1;

@end

