//
//     Generated by class-dump 3.5-MH (64 bit).
//
//  Copyright (C) 1997-2019 Steve Nygard.
//

#import <objc/NSObject.h>

@protocol OS_dispatch_queue;

@interface PLReframeService : NSObject
{
    NSObject<OS_dispatch_queue> *_stateSerializationQueue;
}

@property (readonly, nonatomic) NSObject<OS_dispatch_queue> *stateSerializationQueue; // @synthesize stateSerializationQueue=_stateSerializationQueue;

- (void).cxx_destruct;
- (id)enqueueReframeRequestForAsset:(id)arg1 imageConversionClient:(id)arg2 videoConversionClient:(id)arg3 isOnDemand:(BOOL)arg4 completionHandler:(CDUnknownBlockType)arg5;
- (id)init;

@end

